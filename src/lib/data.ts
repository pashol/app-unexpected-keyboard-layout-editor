                ...
                key: z.array(
                    z.object({
                        $: z.object({
                            width: z.coerce.number().positive().optional(),
                            shift: z.coerce.number().positive().optional(),
                            // key0-key8 format (used by most layouts)
                            ...
                            // Compass format (used by some layouts like latn_qwerty_us)
                            c: z.string().optional(),
    ...
    return {
        ...
        rows: xml.keyboard.row.map((row) => ({
            ...
            keys: row.key.map((key) => ({
                width: key.$.width ?? 1,
                shift: key.$.shift ?? 0,
                key0: str(key.$.key0 ?? key.$.c),
                key1: str(key.$.key1 ?? key.$.nw),
                key2: str(key.$.key2 ?? key.$.ne),
                key3: str(key.$.key3 ?? key.$.sw),
                key4: str(key.$.key4 ?? key.$.se),
                key5: str(key.$.key5 ?? key.$.w),
                key6: str(key.$.key6 ?? key.$.e),
                key7: str(key.$.key7 ?? key.$.n),
                key8: str(key.$.key8 ?? key.$.s),
